...
class DashboardController extends Controller
{
        ...
        if (auth()->user()->getRoleNames()[0] != 'admin') {
            ...
            if (auth()->user()->getRoleNames()[0] == 'polda') {
                ...
                if ($request->polres) {
                    ...
                    $data['chartPolsekAsal'] = $this->chartPolsekAsal($request);
                }
    ...
    public function chartPolres($request, $jenis_pelanggaran = null)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if ($request->tanggal_mulai) $query = $query->where('tgllp', '>=', $request->tanggal_mulai);
            if ($request->tanggal_akhir) $query = $query->where('tgllp', '<=', $request->tanggal_akhir);
            $disiplin_data = $query->first();
            $value->disiplin = is_null($disiplin_data) ? 0 : $disiplin_data->total;
            $query = $query->where(function ($query) {
                $query->whereNotNull('dp3d_bp3kkepp')->orWhere('penyelesaian', 'dihentikan');
            });
            $query = $query->count();
            ...
            if ($request->tanggal_mulai) $query = $query->where('tgllp', '>=', $request->tanggal_mulai);
            if ($request->tanggal_akhir) $query = $query->where('tgllp', '<=', $request->tanggal_akhir);
            $kepp_data = $query->first();
            $value->kepp = is_null($kepp_data) ? 0 : $kepp_data->total;
            $query = $query->where(function ($query) {
                $query->whereNotNull('no_kep')->orWhere('penyelesaian', 'dihentikan');
            });
            $value->kepp_selesai = $query->count();
    ...
    public function chartPolsekAsal($request, $jenis_pelanggaran = null)
    {
        $data = PelanggaranList::groupBy('polsek_terduga', 'polsek_terdugas.name')->leftJoin('polsek_terdugas', 'polsek_terdugas.id', 'pelanggaran_lists.polsek_terduga')
            ->where('is_delete', 0)
            ->orderBy('polsek_terduga', 'asc')
            ->select(DB::raw('count(*) as total'), 'polsek_terdugas.name as nama', 'polsek_terduga');
        ...
        if ($request->polsek) {
            $data = $data->where('polsek_terduga', $request->polsek);
        }
        $data = $data->get();
        foreach ($data as $key => $value) {
            $query = PelanggaranList::groupBy('polsek_terduga', 'polsek_terdugas.name')->leftJoin('polsek_terdugas', 'polsek_terdugas.id', 'pelanggaran_lists.polsek_terduga')
                ->where('is_delete', 0)
                ->whereNotNull('penyelesaian')
                ->where('polsek_terduga', $value->polsek_terduga)
                ->whereNotNull('polres_terduga')
                ->select(DB::raw('count(*) as total'), 'polsek_terdugas.name', 'polsek_terduga');
            if ($request->pangkat) $query = $query->where('pangkat', $request->pangkat);
            if ($request->jenis_kelamin) $query = $query->where('jenis_kelamin', $request->jenis_kelamin);
            if ($request->tanggal_mulai) $query = $query->where('tgllp', '>=', $request->tanggal_mulai);
            if ($request->tanggal_akhir) $query = $query->where('tgllp', '<=', $request->tanggal_akhir);
